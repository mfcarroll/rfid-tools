...
ALL: dict[str, Protocol] = {p.key: p for p in [
    ...
    _p(key="viking", tier=0, family="ask", t55_capable=True,
       ...
       expect="1A337102",
       cu_type="Viking",
       cu_emulate="lf viking econfig -s {slot} --id 1a337102",
    ...
    _p(key="jablotron", tier=0, family="ask", t55_capable=True,
       ...
       expect="0899AABBCC",
       cu_type="Jablotron",
       cu_emulate="lf jablotron econfig -s {slot} --id 0899aabbcc",
       cu_read="lf jablotron read",
       cu_write="lf jablotron write --id 0899aabbcc",
       cu_decode_marker=r"Jablotron ID\s*:",
       cu_expect="0899aabbcc",
       flip_key="Jablotron", flip_expect=None,
    ...
    _p(key="awid", tier=0, family="fsk", t55_capable=True,
       ...
       expect="011d87dd148e281111111111",
       cu_type="AWID",
       cu_emulate="lf awid econfig -s {slot} --raw 011d87dd148e281111111111",
       cu_read="lf awid read",
       cu_write="lf awid write --raw 011d87dd148e281111111111",
       cu_decode_marker=r"AWID FSK2a",
       cu_expect="011d87dd148e281111111111",
       flip_key="AWID",
    ...
    _p(key="noralsy", tier=0, family="ask", t55_capable=True,
       ...
       # ⭐ BENCH-DERIVED 2026-09-15 (run 20260915_191849). The registry's value came from the Chameleon's own arm and the Proxmark writes a different credential; this is what the Proxmark actually put on the tag and read back.
       expect="BB0214FF0110002233070000",
       cu_type="Noralsy",
       cu_emulate="lf noralsy econfig -s {slot} --raw bb0214ff0110002233070000",
       cu_read="lf noralsy read",
       cu_write="lf noralsy write --raw bb0214ff0110002233070000",
       cu_decode_marker=r"Noralsy ASK/Manchester",
       cu_expect="bb0214ff0110002233070000",
       flip_key="Noralsy",
